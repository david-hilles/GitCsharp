...
namespace ConsoleBeep
{
    static class Tetris
    {
        ...
        static void Main()
        {
            //Aqui nesta árte eçe éga a hora e minutos
            var dateVerify = DateTime.Now.ToString("HH:mm");

            Console.WriteLine("informe sua fileira");
            //Pede a informacao da fileira que ele ira executar
            ...
            //Laço de repetiçao que fica rodando enquando não for a hora marcada
            while (dateVerify != timeStart)
            {
                //Atualizamos o valor da variavel "dateVeify" com a hora e minutos atuais
                dateVerify = DateTime.Now.ToString("HH:mm");
            }
            //seta o tempo de incremento entre as execuções
            var startTimerIcrement = 200;
            //Pega um valor de um objeto qualquer e cria uma lista para o mesmo e executar de acordo com a chamada
            switch (fileira)
            {
                ...
                //EM caso de não ser informado uam string valida para nosso menu de opçoes, ele executa esse cara
                default: { Console.WriteLine("Nâo foi uma escolha valida dentro do menu de opçoes."); } break;
